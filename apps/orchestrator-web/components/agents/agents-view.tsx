        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-2.5">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold text-[color:var(--foreground)]">Agents</h1>
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant={providersLoaded > 0 && workflowsLoaded > 0 ? 'success' : 'muted'}>
                {providersLoaded > 0 && workflowsLoaded > 0
                  ? 'Headless matrix ready'
                  : providersLoaded > 0 || workflowsLoaded > 0
                    ? 'Runtime inventory partial'
                    : 'Runtime inventory empty'}
              </Badge>
          ...
          <tbody className="divide-y divide-[color:var(--line)]">
            {providerItems.map((p) => (
              <tr key={p.id} className="bg-black/5 hover:bg-white/[0.03]">
                ...
                <td className="px-4 py-3.5 text-[color:var(--foreground-soft)]">{p.version}</td>
                <td className="px-4 py-3.5">
