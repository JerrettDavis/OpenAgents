...
function StageTasks({ jobId, stageId }: { jobId: string; stageId: string }) {
  ...
  if (tasks.length === 0)
    return (
      <p className="px-4 py-2 text-xs text-zinc-600">No tasks for this stage.</p>
    );
      ...
      >
        ...
        <span className="text-xs text-zinc-600">
          {stage.started_at_utc
            ? formatElapsed(
                stage.started_at_utc,
                stage.finished_at_utc ?? undefined
              )
            : ""}
        </span>
